...
{
    DecideAndMoveToRightDevice(*this, gradients, smoothedGradients);
    ...
    DISPATCH_MATRIX_ON_FLAG(&gradients, nullptr,
        ...
        { 
            if (momentum != 0) gradients.m_CPUSparseMatrix->NormalGrad(*m_CPUMatrix, momentum, unitGainMomentum);
            ScaleAndAdd(-learnRatePerSample, gradients, *this);
        },
        { 
            if (momentum != 0) gradients.m_GPUSparseMatrix->NormalGrad(*m_GPUMatrix, momentum, unitGainMomentum);
            ScaleAndAdd(-learnRatePerSample, gradients, *this);
        });
}
    ...
    DISPATCH_MATRIX_ON_FLAG(&gradients, nullptr,
        ...
        { /* CPU sparse */
            if (momentum != 0)
            {
                ...
                gradients.m_CPUSparseMatrix->NormalGrad(*m_CPUMatrix, momentum, unitGainMomentum);
                ScaleAndAdd(-momentum, smoothedGradients, *this);
        ...
        { /* GPU sparse */
            if (momentum != 0)
            {
                Matrix<ElemType> gradientCache(gradients.GetDeviceId());
                gradientCache.AssignValuesOf(gradients);
                gradients.m_GPUSparseMatrix->NormalGrad(*m_GPUMatrix, momentum, unitGainMomentum);
                ScaleAndAdd(-momentum, smoothedGradients, *this);
                ScaleAndAdd(-unitGainFactor * learnRatePerSample, gradientCache, *this);
            }
        });
