...
%typemap(cscode) CNTK::Value %{
    ...
    // Create Value based on Dense Sequences
    ...
    public static Value Create<T>(System.Collections.Generic.List<CNTK.SequenceOneHotVector> sequences, DeviceDescriptor computeDevice)
    {
        if (sequences.Count == 0)
        {
            throw new System.ArgumentException("No data is provided.");
        ...
        var list = new System.Collections.Generic.List<System.Collections.Generic.List<uint>>();
        var vocabSize = sequences[0].VocabSize;
        foreach (var seq in sequences)
        {
            if (seq.VocabSize != vocabSize)
            {
                throw new System.ArgumentException(string.Format("The input has different vocabuaray size. {0} vs {1}", vocabSize, seq.VocabSize));
            }
